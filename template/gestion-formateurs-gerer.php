...
<table class = "temporary-table">
    <?php
       for($i = 0; $i < $resultNumber; $i++){
            ...
            echo '<td style = "min-width:250px" class="border-bottom-0">
             <a href="'.get_site_url().'/profile/'.$getAllFormsDemandes[$i]->user_login.'" target = "_blank" data-user = "'.$getAllFormsDemandes[$i]->user_login.'" style = "background:#2878EB;color:white" class="action-btn-allow btn">Profil</a>
             <a href="#" data-user = "'.$getAllFormsDemandes[$i]->user_login.'" class="action-btn-revoke btn btn-danger">Révoquer</a><br/>
             <a href="#" data-user = "'.$getAllFormsDemandes[$i]->user_login.'" style = "margin-top:5px" class="action-btn-paid btn btn-success">Payer</a>
             </td>
...
+"<tr style = \"background:#2878EB;color:white\"><th>Photo de profil</th><th>Nom</th><th>E-mail</th><th>Telephone</th><th>Siret</th><th>Iban</th><th>Bic</th><th>Pays</th><th>Statut</th><th>Action</th></tr></thead><tbody class = \"formateurs-demandes-table\">"
...
setTimeout(()=>{
    let paidBtn = document.querySelectorAll(\'.action-btn-paid\');
    let revokeBtn = document.querySelectorAll(\'.action-btn-revoke\');
    for(let i = 0; i < paidBtn.length; i++){
        paidBtn[i].addEventListener(\'click\',()=>{
            let xhttp = new XMLHttpRequest();
            xhttp.onreadystatechange = function() {
              if (this.readyState == 4 && this.status == 200) {
                paidBtn[i].parentElement.parentElement.remove();
              }
            };
            xhttp.open("POST", "", true);
            xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
            xhttp.send("formateurAction=paid&statut-user-login="+paidBtn[i].getAttribute(\'data-user\'));
        });
        revokeBtn[i].addEventListener(\'click\',()=>{
            raisonStatut = prompt("Entrez la raison du refus");
            if(raisonStatut != null){
                let xhttp = new XMLHttpRequest();
                xhttp.onreadystatechange = function() {
                  if (this.readyState == 4 && this.status == 200) {
                    revokeBtn[i].parentElement.parentElement.remove();
                  }
                };
                xhttp.open("POST", "", true);
                xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
                xhttp.send("statutRaison="+raisonStatut+"&formateurAction=revoke&statut-user-login="+revokeBtn[i].getAttribute(\'data-user\'));
                raisonStatut = null;
            }
